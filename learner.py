...
class Learner(BaseAutoEncoderLearning):
    def __init__(self,
        ...
        if self.learner_config["layer"] == 'linear': 
            # Load the classifier
            self.classifier = load_classifier(layer='linear')
        elif self.learner_config["layer"] == 'nlrl':
            # Load the classifier
            self.classifier = load_classifier(layer='nlrl')
        else:
    ...
    def _train_epoch(self, train=True):
        ...
        for i, data in enumerate(self.train_data):
            ...
            self.fid_metric.update(images, is_real=True)
            self.fid_metric.update(decoded_images.detach(), is_real=False)
